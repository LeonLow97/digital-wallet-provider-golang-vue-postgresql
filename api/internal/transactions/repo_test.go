...
func TestGetByUserId(t *testing.T) {
	...
	transactions, err := testRepo.GetByUserId(context.Background(), "sender")
	assert.NoError(t, err)

	// Assert
	assert.Len(t, transactions.Transactions, 3)
	assert.Equal(t, "sender", transactions.Transactions[0].SenderName)
	assert.Equal(t, "beneficiary", transactions.Transactions[0].BeneficiaryName)
}
